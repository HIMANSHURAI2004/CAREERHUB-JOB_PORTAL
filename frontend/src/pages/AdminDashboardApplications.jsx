...
function AdminDashboardApplications() {
    ...
    const getApplicationsData = async () => {
        ...
        try {
          const response = await fetch('http://localhost:3000/api/v1/user/admin-dashboard', {
            ...
            body: JSON.stringify({ modelName: 'applications' }),
            credentials: 'include',
          });
          if (!response.ok) {
          ...
          setApplicationEntries(responseData.data.slice(0, 5));
        } catch (error) {
        ...
        <div>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 gap-4 mt-4">
                    <div>
                        <h2 className='text-2xl lg:text-4xl lg:mx-10 md:mx-8 mx-2 font-semibold'>Applications Data</h2>
                    </div>
                    ...
                    (
                        <Card key={index} className='lg:mx-16 md:mx-8 mx-2 my-3'>
                            <div className='bg-[#294f7c] text-white rounded-t-lg flex items-center justify-between px-4 py-2'>
                                <h2 className="text-lg font-semibold">Application: {index +1}</h2>
                                <h2 className='hidden sm:block'>{entry._id}</h2>
                            </div>
                            <CardContent>
                                <div className='flex flex-col gap-y-1 pt-2'>

                                    <p><span className='font-medium lg:hidden md:hidden sm:hidden'>ID: </span><span className='text-blue-800 lg:hidden md:hidden sm:hidden'>{entry._id}</span></p>
                                    <p><span className='font-medium'>Applicant: </span><span className='text-blue-800'>{entry.applicant}</span></p>
                                    <p><span className='font-medium'>Job: </span><span className='text-blue-800'>{entry.job}</span></p>
...
export default AdminDashboardApplications;
